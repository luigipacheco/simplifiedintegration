...
void loop() {
  ...
  if (currentMillis - lastSensorUpdate >= sensorUpdateInterval) {
    ...
    MotorSpeed = constrain(rawMotorSpeed, 0, 1000);
...
void updateOLED(void) {
  ...
  if (wifiConnected) {
    ...
    display.drawBitmap(109, 0, image_wifi_full_bits, 19, 16, SH110X_WHITE);
    display.setCursor(1, 100); // Position for IP address
    display.print(WiFi.localIP());
  } else {
    // Display WiFi not connected icon
    display.drawBitmap(109, 1, image_wifi_not_connected_bits, 19, 16, SH110X_WHITE);
  ...
  display.drawBitmap(3, 21, image_weather_temperature_bits, 16, 16, SH110X_WHITE);
  ...
  display.drawBitmap(2, 64, image_Restoring_bits, 38, 32, SH110X_WHITE);
   if ( MotorSpeed >= 1 ){
      display.drawBitmap(46, 2, image_poopface_bits, 29, 14, SH110X_WHITE);
    }
    else{
      display.drawBitmap(46, 2, image_FaceNormal_bits, 29, 14, SH110X_WHITE);
    }
    display.setTextSize(1);
    display.drawLine(0, 97, 127, 97,  SH110X_WHITE);
    display.drawLine(0, 110, 127, 110,  SH110X_WHITE);
